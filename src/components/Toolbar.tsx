  ...
  return (
    <div className="absolute top-6 left-1/2 -translate-x-1/2 z-10 bg-white/80 backdrop-blur-md shadow-xl rounded-full flex items-center p-3 border border-slate-200/50 gap-6 px-12 whitespace-nowrap flex-nowrap">
      ...
      <div className="relative group/add">
        ...
        <div className="absolute top-[calc(100%-2px)] left-0 w-64 bg-white shadow-2xl border border-slate-200 opacity-0 invisible group-hover/add:opacity-100 group-hover/add:visible transition-all flex flex-col overflow-hidden divide-y divide-slate-100 rounded-xl rounded-tl-none z-20 whitespace-normal">
           <button onClick={() => onAddNode('blank')} className="text-left px-4 py-3 hover:bg-slate-50 text-sm font-medium text-slate-700 transition-colors">Blank Node</button>
           <button onClick={() => onAddNode('sticky_note')} className="text-left px-4 py-3 hover:bg-yellow-50 text-sm font-semibold text-yellow-700 transition-colors flex items-center gap-2">
               ...
               <div className="max-h-48 overflow-y-auto divide-y divide-slate-50">
                 {customTemplates.map(tpl => (
                   <div key={tpl.id} className="group/tpl flex items-center justify-between hover:bg-amber-50/50 transition-colors">
                     <button 
                        onClick={() => onAddNode('custom', tpl.data)} 
           ...
           <button onClick={() => onAddNode('organic_search')} className="text-left px-4 py-3 hover:bg-emerald-50 text-sm font-medium text-emerald-700 transition-colors border-b border-slate-50">Organic Search</button>
           <button onClick={() => onAddNode('paid_search')} className="text-left px-4 py-3 hover:bg-amber-50 text-sm font-medium text-amber-600 transition-colors border-b border-slate-50">Paid Search Ads</button>
           <button onClick={() => onAddNode('paid_social')} className="text-left px-4 py-3 hover:bg-purple-50 text-sm font-medium text-purple-700 transition-colors border-b border-slate-50">Paid Social Ads</button>
           <button onClick={() => onAddNode('landing_page')} className="text-left px-4 py-3 hover:bg-blue-50 text-sm font-medium text-blue-700 transition-colors border-b border-slate-50">Landing Page</button>
           <button onClick={() => onAddNode('gated_content')} className="text-left px-4 py-3 hover:bg-slate-100 text-sm font-medium text-slate-800 transition-colors border-b border-slate-50">Gated Content LP</button>
           <button onClick={() => onAddNode('product_page')} className="text-left px-4 py-3 hover:bg-blue-100 text-sm font-medium text-blue-900 transition-colors border-b border-slate-50">Product Page</button>
           <button onClick={() => onAddNode('email_campaign')} className="text-left px-4 py-3 hover:bg-indigo-50 text-sm font-medium text-indigo-700 transition-colors border-b border-slate-50">Email Campaign</button>
           <button onClick={() => onAddNode('ecommerce_checkout')} className="text-left px-4 py-3 hover:bg-rose-50 text-sm font-medium text-rose-700 transition-colors">E-Commerce</button>
        </div>
      </div>
